...
def generate_problem_data(**kwargs):
    ...
    container_length : average containers length (default 50)
    container_width : average containers width (default 50)
    ...
    container_length = kwargs.get("container_length", DEFAULT_MEAN_CONTAINER_LENGTH)
    container_width = kwargs.get("container_width", DEFAULT_MEAN_CONTAINER_WIDTH)
    ...
    deviation_w_val = max(round(items_length * 0.8), 1)
    deviation_h_val = max(round(items_width * 0.8), 1)
